...
- **产品名称**：已定为 **「小荧」**；实现时需替换全站 `metadata`、Landing 标题等（见阶段 P0-E）。
...
**说明**：根据文档内批注已做调整——产品名 **小荧**；法务文案、热线、Bot 话术、案例库规则、自助包定稿等事项 **内容待定但可先占位**；**不**单列「部署目标确认」任务；**不**将「质量安全运维整段」「向量检索升级整段」列入本版里程碑（见 **§12.3**）。

### 阶段 0：前置决策与内容准备

- [ ] **小荧**：列出需替换文案的页面清单（`metadata`、Landing、顶栏、页脚），与实现 PR 对齐勾选
