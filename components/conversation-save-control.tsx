    ...
    return (
      <SignInButton mode="modal">
        <Button variant="ghost" size="sm">
          ...
          <span className="hidden sm:inline">登录后保存</span>
        </Button>
      </SignInButton>
    )
  }
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {historyEnabled ? "关闭未来对话保存？" : "开启云端对话保存？"}
          </DialogTitle>
          <DialogDescription className="space-y-2 text-left leading-6">
            {historyEnabled ? (
              <>
                <span className="block">关闭后，新消息将不再写入云端。</span>
                <span className="block">已有记录不会自动删除，可在对话记录页单独清空。</span>
              </>
            ) : (
              <>
                <span className="block">从下一条消息开始，对话会加密保存到 Cloudflare D1，并关联当前 Clerk 账号。</span>
